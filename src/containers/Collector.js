...
export default class Collector extends React.Component {
    ...
    onChange = () => {
        ...
        fetch(`http://localhost:3001/waste_items/${this.state.waste_item_id}`, {
            method: 'PUT',
            headers: {
    ...
    render() {
        var wasteItems = this.state.wasteItems.filter(item =>
            item.state.includes('open')).map((item) => {
                ...
                return (
                    <div key={item.id} className={"card"}>
                        ...
                        <div className={"card_title"}>
                            ...
                            {stateTxt == "open" &&
                                <button data-id={item.id} onClick={this.bookItem}>Book Item</button>
                            }
                        </div>
                        <div className="card_content">
                            ...
                            <div className="resident">
                                ...
                                <h5>Resident Name: {item.resident.first_name + ' ' + item.resident.surname}</h5>
                            ...
                            <div>
                                <div id="map">
                                    <iframe
                                        ...
                                        height='200'
                                        id='mapcanvas'
                                        src={`https://maps.google.com/maps?q=${item.resident.postcode}&Roadmap&z=10&ie=UTF8&iwloc=&output=embed`}
                                        frameborder='0'
                                        scrolling='no'
                                        marginheight='0'
        ...
        var wasteItemsBooked = this.state.wasteItems.filter(item =>
            item.state.includes('booked') && (item.collector_id === 1)).map((item) => {
                const stateTxt = item.state
                return (
                    <div key={item.id} className={"card"}>
        ...
        var wasteItemsApproved = this.state.wasteItems.filter(item =>
            item.state.includes('approved') && (item.collector_id === 1)).map((item) => {
                const stateTxt = item.state
                return (
                    <div key={item.id} className={"card"}>
        ...
        return (
            <div>
                ...
                <div>
                    <h3>Waste Items Disposed Successfully! {wasteItemsApproved.length}</h3>
                    <br></br>
                    {wasteItemsApproved}
                    <br></br>
                </div>
